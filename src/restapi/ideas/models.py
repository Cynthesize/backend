...
class Idea(models.Model):
    ...
    REQUIRED_FIELDS = ['idea_name', 'owner',
                       'description', 'require_assistance']
...
class Pinned_ideas(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.DO_NOTHING, related_name='pinned_ideas')
    idea_list = ArrayField(models.CharField(
        max_length=20, blank=True), default=list)


class Upvoted_ideas(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.DO_NOTHING, related_name='upvoted_ideas')
    idea_list = ArrayField(models.CharField(
        max_length=20, blank=True), default=list)
